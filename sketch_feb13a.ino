...
void encoding_1(){
  ...
  if(digitalRead(encoder_A_1) == HIGH){
    if(digitalRead(encoder_B_1) == LOW) Motor->encoder ++;
    else Motor->encoder --;
  }else{
    if(digitalRead(encoder_B_1) == HIGH) Motor->encoder ++;
    else Motor->encoder --;
  }
}
...
void encoding_2(){
  ...
  if(digitalRead(encoder_A_2) == HIGH){
    if(digitalRead(encoder_B_2) == LOW) Motor->encoder ++;
    else Motor->encoder --;
  }else{
    if(digitalRead(encoder_B_2) == HIGH) Motor->encoder ++;
    else Motor->encoder --;
  }  
}
